...
/**
 * GET /api/watchlist
 * 12개 Watchlist 종목 데이터 + 퀀트 메트릭 + 액션 시그널 반환.
...
export async function GET(): Promise<NextResponse<WatchlistResponse | { error: string }>> {
  try {
    ...
    // 12개 종목 병렬 fetch + 퀀트 계산
    const data = await buildTickerResponse(WATCHLIST_TICKERS, spChangePercent);
    ...
    console.error("[/api/watchlist] critical failure:", error);
    return NextResponse.json(
